...
```
data/
  site.yaml              ← общи настройки на сайта (заглавие, иконка, фон, контакти, футър)
  trips/
    bulgaria-varna.yaml  ← един файл = една екскурзия
    greece-alexandroupolis.yaml
    turkey-istanbul.yaml
    ...
...
contact:
  ...
  email: info@kztravel.bg
  address: 'ул. Примерна 1, гр. Казанлък'
footer:
  registration: Свидетелство за регистрация на туроператор номер РК-00-00000
  company: ФИРМА ЕООД
```
...
| `contact` | Телефон, имейл, адрес и карта за страницата „Контакти" |
| `footer.registration` | Текст за регистрация на туроператор в долния колонтитул |
| `footer.company` | Име на фирмата в долния колонтитул |
...
1. Отворете съответния файл в `data/trips/` (напр. `bulgaria-varna.yaml`).
2. Променете текста, цените или датите.
3. Запазете файла и качете промяната в GitHub.

**Смяна на цена или наличност:** намерете секцията `dates` и променете `price`, `priceBgn` или `status`:

```yaml
dates:
  - date: 2026-07-15
    price: 420          # цена в евро
    priceBgn: 822       # цена в лева
    status: available   # available | lastSpots | soldout
```

**Маркиране като изчерпано:** сложете `status: soldout` на съответната дата.

**Отстъпка (по избор):** добавете `discountedPrice` и `discountedPriceBgn` към датата:
...
```yaml
  - date: 2026-08-01
    ...
    priceBgn: 880
    discountedPrice: 399
    discountedPriceBgn: 781
    status: available
```

> **Забележка:** По-старото поле `available: true/false` все още работи, но е остаряло. Използвайте `status` вместо него.
...
Най-лесно е да **копирате** съществуващ файл (напр. `bulgaria-varna.yaml`), да го преименувате и да смените съдържанието.
...
| `dates[].priceBgn` | да | Цена в лева (само число, без лв.) |
| `dates[].status` | да | `available` = има места, `lastSpots` = последни места, `soldout` = изчерпано |
| `dates[].discountedPrice` | не | Намалена цена в евро (по избор) |
| `dates[].discountedPriceBgn` | не | Намалена цена в лева (по избор) |
| `dates[].available` | не | **Остаряло** — използвайте `status` вместо това |
...
**Как се изчислява цената на картата:** сайтът показва цената и датата на **най-близкото бъдещо заминаване**, за което `status` е `available` или `lastSpots`.
...
Проверете `country` - трябва да е на латиница, малки букви, напр. `italy`, не `Италия`. Ако държавата е нова за сайта, програмистът трябва да добави превод на името ѝ в `src/lib/strings.ts` (обект `countryLabels`). Без това името се показва автоматично на латиница.
